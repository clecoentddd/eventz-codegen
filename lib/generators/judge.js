...
import { toValidFunctionName, toKebabCase } from '../helpers.js';
...
${command.fields.map(field => `        ${field.name}: intent.data.${field.name},`).join('\n')}
  ...
  // };
      ...
      if (command.title === 'Order Cappuccino') {
        ...
        const sliceNameKebab = toKebabCase(slice.name);
        const modulePath = `slices/state-change/${slice.name}/judge.js`;
        const fileContent = `
